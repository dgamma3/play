<template>
  <div>
          ...
          <div class="account-user">
            ...
            <div class="account-info">
              <div class="account-name">
                {{ user.name }}
                <span   style="color:#cccccc;line-height:60px;padding-right: 10px;font-size: 12px;" v-if="user.shimin_status == 2">{{user.username}}[已实名]</span>
                <span   style="color:#cccccc;line-height:60px;padding-right: 10px;font-size: 12px;" v-if="user.shimin_status != 2"> [未实名]</span>
              </div>
        ...
        <div v-if="type == '' || type == 'all'" class="account-items">
          ...
          <template v-else>
            <div class="account-items-items">
              <div :class="'live-item live-item-bg live-item-bg-'+color(item.ornaments.color, 4)+' '+ [item.selected ? 'active' : '']" v-for="(item, index) in ornaments"
                @click="ornaments_switch(index)" v-if="item.ornaments" :key="index">
          ...
          <table class="level-table">
            ...
            <tbody>
              ...
              <tr class="level-table-row2" v-for="(item, index) in recharge_record" :key="index">
                ...
                <td class="level-table-cell ">
                  <span class="live-item-price price_box "><child-component v-html="symbol"></child-component> {{ item.money }}</span>
                </td>
                <td class="level-table-cell">
          ...
          <table class="level-table">
            ...
            <tbody>
              ...
              <tr class="level-table-row2" v-for="(item, index) in partner_history" :key="index">
                ...
                <td class="level-table-cell">
                  <span class="zhubo_price_box"><child-component v-html="symbol"></child-component> {{ item.money }}</span>
                </td>
                <!-- <td class="level-table-cell">
    ...
    <el-dialog class="link-dialog" title="兑换金币" append-to-body :visible.sync="diamond_to_gold_show" :close-on-click-modal="false"
      :lock-scroll="false">
      <div class="link-info">
    ...
    <el-dialog class="link-dialog" title="修改交易链接" append-to-body :visible.sync="link_page" :close-on-click-modal="false"
      ...
      <div class="link-info">
        <el-input class="recharge-input" v-model="user.transaction_link" placeholder="请输入交易链接">
          <a slot="append" class="recharge-btn recharge-btn2"  href="https://steamcommunity.com/id/me/tradeoffers/privacy#trade_offer_access_url"  target="_blank">获取交易链接</a>
        </el-input>

        <el-input class="recharge-input" v-model="vif" placeholder="请输入验证码" v-if="user.phone">  <button slot="append" class="recharge-btn recharge-btn2" @click="get_vif">获取验证码</button>  </el-input>
  ...
  export default {
    ...
    methods: {
      ...
      diamond_to_gold(){
        ...
        let loading = this.$loading({  spinner: 'el-icon-loading',  background: 'rgb(0 0 0 / 0%)' });
...
<style scoped>
 .account-balance  /deep/  .symbol{width: 28px; padding-top: 3px; margin-right: 10px; float: left;}
 .account-items-btn  /deep/  .symbol{width: 16px;  margin-top: -2px; margin-right: 5px;margin-left: 10px; float: left;}



 .live-item-price { float: right; display:flex; justify-content:center; align-items:center;  margin-top: 3px; margin-bottom: 20px; }
