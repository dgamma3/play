  ...
  <div>
    ...
    <template v-else>
      <div class="case">
        <div class="case-inner">
          ...
          <template v-else>
            <div class="case-price price_box "><child-component v-html="symbol"></child-component> {{ total }}</div>
          </template>
        ...
        <template>
          <div class="lottery-warp" ref="roulette">
            <div class="lottery"  :key="item.id" v-for="item in box_ornaments_arr">
            <img
            ...
            <div  class="containers mobile">
              ...
              :style="
                ...
                "
              >
                <div :key="index" v-for="(item, index) in item.box_ornaments"
                class="item" :style="{'left':(index*184.8)+baifenbi}">

                  <div
                    :class="'img img'+item.color"
                  >
                    <img
                      :src="img_url + item.img"
                      class="item-icon"
              ...
              <div class="case-items">
                <div class="case-inner">
                  <div class="case-items-wrap">
                    ...
                    <div  v-for="(item2, index2) in item.arr"
                      :class="'live-item live-item-bg live-item-bg-'+color(item2.color, 4)"
                       :key="index2"
                    >
                      <div class="live-item-probability">
                        <div>机会: {{ item2.probability }}%</div>
                        <div class="live-item-price price_box ">
                          <template v-if="item2.appid == 'candy'">
                            1
                          </template>
                          <template v-else>
                            <child-component v-html="symbol"></child-component> {{ item2.open_price }}
                          </template>
                        </div>
                      </div>
                      <div class="live-item-roll">
                        <div>Roll: {{ item2.min_roll }}-{{ item2.max_roll }}</div>

                      </div>
                      <div :class="color(item2.color, 3)">
                        <div class="o-item">
                          <div class="o-body">
                        ...
                        <div class="o-abrasion" v-if="item2.appid == 730">
                          {{ abrasion(item2.abrasion)
                          }}{{ quality(item2.quality, 3) }}
                        </div>
              ...
              >
                ...
                <el-table-column prop="img" label="图片"  >
                  <template #default="scope">
                    <img style="width:5rem;height:5rem;" :src="img_url + scope.row.img" alt="" />
                  </template>
                </el-table-column>
              </el-table>
                    ...
                    <div :class="color(item.color, 3)">
                      ...
                      <div>
                        <span class="live-item-price"
                          ><child-component v-html="symbol"></child-component> {{ item.open_price }}</span
                        >
                        <span
                          style="
                            margin-left: 10px;
    ...
    >
      ...
      <div class="result-buttons">
        ...
        <button class="result-btn result-btn2 price_box " @click="sell">
          出售 <img src="../../../assets/img/zs.png" style="width:17px;height:17px;margin: 0 1px 0 6px;">{{ ornaments_arr[0].open_price }}
        </button>
      </div>
    </el-dialog>
    <el-dialog
    ...
    >
        ...
        >
          ...
          <div :class="color(item.color, 3)">
            ...
            <div class="live-item-price">
              ...
              <template v-else>
                <span class="price_box "><child-component v-html="symbol"></child-component> {{ item.open_price }}</span>
              </template>
            </div>
            <div class="o-name">{{ item.name }}</div>
            <div class="o-abrasion" v-if="item.appid == 730">
      ...
      <div class="result-buttons">
        <button class="result-btn" @click="again">再试一次</button>
        <button class="result-btn result-btn2 price_box " @click="sell">
          出售所有 <img src="../../../assets/img/zs.png" style="width:17px;height:17px;margin: 0 1px 0 6px;">{{ ornaments_arr_price_total }}
        </button>
        <button
          :class="
        ...
        if (res.code == 200) {
          ...
          setTimeout(() => {
            for (let x in box_ornaments_arr) {
              ...
              //正确位移像素位置
              console.log('正确的位置：'+this.roulette_list_item_weight * random)
              translate_x =
              screen_width / 2 -
    ...
    sell() {
      ...
      ttPost("/api.php/user/sell", { record_id: record_id }, (res) => {
        ...
        //this.user.store_bean = Number(res.data.store_bean).toFixed(2);
           this.$emit('user_store_bean', {  user_store_bean: res.data.store_bean  });
          this.$message({
            message: res.msg,
            type: "success",
          });
...
<style scoped>
 ...
 .case-price{   display:flex; justify-content:center; align-items:center;  margin-top: 3px;  }
 .case-price /deep/  .symbol{ margin-left: 10px; width: 16px; padding-top: 3px; margin-right: 5px;  }



 .result-btn2{   display:flex; justify-content:center; align-items:center;  margin-top: 3px;  }
 .result-btn2 /deep/  .symbol{ margin-left: 10px; width: 16px; padding-top: 3px; margin-right: 5px;  }
</style>
