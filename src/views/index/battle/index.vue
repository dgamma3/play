              ...
              <div class="luck-boxs" v-for="(item, index) in box" v-if="item.price_status && item.name_status">
                <div class="luck-card case-discount" @click="openBox(item.id, item.name)">
                  ...
                  <div class="luck-card-price ">
                    <img src="@/assets/img/jb.png" style="width:20px;height:20px;margin-right:10px;" />
                    {{ item.price }}
                  </div>
                </div>
              </div>
        ...
        <div class="battle-content">
          <div class="rankings">
            <div class="ranking-no-one-bg">
              ...
              <div class="ranking-no-one  shine-show-star">
                <div class="ranking-no-one-content">
                  <div class="userWin">
                    <img :src="
                        yestday_ranking_one.user
                          ? baseUrl +
                            '/public/uploads/' +
                            yestday_ranking_one.user.portrait
                          : '/static/img/avatar.jpg'
                      " alt="" class="avatar" /><!---->
                  </div>
                </div>
                <div class="color-c3 mb6 star-text">
                  {{
                    yestday_ranking_one.user
                      ? yestday_ranking_one.user.name
                      : "神秘用户"
                  }}
                </div>
                <div class="color-e9">
                  <span class="price">
                    {{ yestday_ranking_one.count_num }}
                  </span>
                </div>
              </div>
              <div class="jlsm">
                <div style="display: flex; align-items: center">
                  <el-popover trigger="hover" :width="300"
                    popper-style="padding: 20px;background: rgba(33,19,66,.9) !important;" popper-class="popover-box">
                    <template #reference>
                      ...
                      <div class="demo-rich-conent">
                        <div v-for="(item, index) in prizes" class="prize-box">
                          <div class="prize-index">
                            <img v-if="item.value == 1" src="@/assets/img/prize_01.png"
                              style="width:25px;height:26px;" />

                            <img v-else-if="item.value == 2" src="@/assets/img/prize_02.png"
                              style="width:25px;height:26px;" />

                            <img v-else-if="item.value == 3" src="@/assets/img/prize_03.png"
                              style="width:25px;height:26px;" />
            ...
            <!--                       <div class="rank-title-txt dis-cen">
              <div @click="switch_ranking_type(0)" :class="
                  'table-title-one ' + (ranking_type == 0 ? ' active' : '')
                ">
                今日战力排行榜
              </div>
                           <div
                @click="switch_ranking_type(1)"
                :class="
                  'table-title-two ' + (ranking_type == 1 ? ' active' : '')
                "
              >
                今日胜利次数榜
              </div>
            ...
            <div class="ranking-list relative">
              ...
              <div class="filght_number">
                <span class="filght-mine">我的今日战力值</span><span> {{ my_num }}</span>
              </div>
            </div>
          </div>
          <transition-group tag="div" class="battle-list" enter-active-class="animate__animated animate__flipInX">
            <div :key="item.battle_ids" v-for="(item, index) in battle" class="battle-item"
              @click="openBattle(item.battle_id)">
              <section :class="{
                  'battle-box': true,
                  'battle-box-wait-1':
                    item.battle_type === '1' && item.status == '1', // 欧皇
                  'battle-box-wait-2':
                    item.battle_type !== '1' && item.status == '1', // 非酋
                ...
                <div class="battle-box-head">
                  ...
                  <div class="battle-box-title">
                    <div v-if="item.battle_type == '1' && item.status != '3'">
                      欧皇
                    </div>
                    <div v-if="item.battle_type != '1' && item.status != '3'">
                      非酋
                ...
                <section class="battle-box-main">
                  <section class="tx">
                    <div class="battle-box-seat" :class="{ 'battle-box-seat-four': item.total > 3 }">
                      <img v-if="
                          user_index !== 0 && item.total > 3 && isSmallScreen
                        " src="https://hicsgo.oss-cn-hangzhou.aliyuncs.com/fight/vs.png" alt="" class="fourVs" />
                      <div v-for="(user_item, user_index) in item.user" :key="user_index">
                        <img v-if="
                            user_index !== 0 && item.total < 4 && isSmallScreen
                          " src="https://hicsgo.oss-cn-hangzhou.aliyuncs.com/fight/vs.png" alt="" class="vs" />
                        <img v-if="user_index !== 0 && !isSmallScreen"
                          src="https://hicsgo.oss-cn-hangzhou.aliyuncs.com/fight/vs.png" alt="" class="vs" />
                        <div class="rants">
                          <img :src="repair(user_item.portrait)" alt="" class="txdetal" />
                          <div v-if="user_item.type2==2" class="zhubo">主播</div>
                        </div>
                      </div>

                      <div v-if="Number(item.difference) > 0" v-for="(difference_item,
                        difference_index) in Array.from(
                          new Array(Number(item.difference))
                        )" :key="difference_index">
                        <img v-if="item.total < 4 && isSmallScreen"
                          src="https://hicsgo.oss-cn-hangzhou.aliyuncs.com/fight/vs.png" alt="" class="vs" />
                        <img v-if="!isSmallScreen" src="https://hicsgo.oss-cn-hangzhou.aliyuncs.com/fight/vs.png" alt=""
                          class="vs" />
                        <div class="rants">
                          <router-link :to="'/battle/' + item.battle_id">
                            <img v-if="
                                item.battle_type == '1' && item.status != '3'
                              " src="/static/img/lu2.png" alt="" class="bg" />
                            <img v-if="
                                item.battle_type != '1' && item.status != '3'
                              " src="/static/img/lu.png" alt="" class="bg" />
                            <img v-if="item.status == '3'" src="/static/img/lu3.png" alt="" class="bg" />
                          </router-link>
                        </div>
                      </div>
                    </div>
                    <!-- <div class="battle-box-seat" v-if="item.user.length < 4">
                      <div
                        v-for="(user_item, user_index) in item.user"
                        :key="user_index"
                      >
                        <img
                          v-if="user_index !== 0"
                          src="https://hicsgo.oss-cn-hangzhou.aliyuncs.com/fight/vs.png"
                          alt=""
                          class="vs"
                        />
                        <div class="rants">
                          <img
                            :src="repair(user_item.portrait)"
                            alt=""
                            class="txdetal"
                          />
                        </div>
                      ...
                      <div
                        v-if="Number(item.difference) > 0"
                        v-for="(difference_item,
                        difference_index) in Array.from(
                          new Array(Number(item.difference))
                        )"
                        :key="difference_index"
                          ...
                          <router-link :to="'/battle/' + item.battle_id">
                            ...
                            <img
                              v-if="
                                item.battle_type != '1' && item.status != '3'
                              "
                              src="/static/img/lu.png"
                              alt=""
                              class="bg"
                            />
                            <img
                              v-if="item.status == '3'"
                              src="/static/img/lu3.png"
                              alt=""
                              class="bg"
                            />
                          </router-link>
                ...
                <section class="bottosbox">
                  <div class="pattern">
                    <p>
                      ...
                      <span class="price_box">
                        <template v-if="item.deduction > 0">
                          ...
                          {{ (item.open_price - item.deduction).toFixed(2) }}
                        </template>
                        <template v-else>
                          <child-component v-html="symbol"></child-component>
                          {{ item.open_price }}
                    ...
                    <div>
                      <router-link style="display:flex;color:rgb(248 181 0);white-space: nowrap;"
                        :to="'/battle/' + item.battle_id">
                        <span>查看</span>
                        <img src="https://hicsgo.oss-cn-hangzhou.aliyuncs.com/fight/play-look.png" alt=""
                          style="margin-left: 5px; float:right; margin-top: 2px; margin-bottom: 2px;" />
                      </router-link>
                    </div>
                  </div>
                </section>
